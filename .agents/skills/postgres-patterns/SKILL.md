...
|----------|-------------|-------|
| IDs | `bigint` or `uuid` | `int`, unindexed text IDs |
...
**RLS Policy (Supabase auth example):**
```sql
...
```sql
-- Review only. Do not run blindly; tune for the deployment environment.

-- Connection limits (adjust for RAM, pooler, and workload)
