...
export function OpportunitiesPage() {
  ...
  return <Container className="py-12">
    ...
    <div className="mt-6">
      ...
      {open.length > 0 ? <div className="grid gap-4 md:grid-cols-2">{open.map((item) => <article id={item.id} key={item.id} className="rounded-lg border border-line bg-surface p-5"><div className="flex flex-wrap items-center justify-between gap-2 text-meta"><span className="font-semibold text-accent">{item.type}</span><span className="text-ink-muted">Deadline: {formatDate(item.deadline)}</span></div><h3 className="mt-3 text-lg font-semibold leading-snug text-ink">{item.title}</h3><p className="mt-2 text-sm text-ink-muted">{item.organization} · {item.region}</p><p className="mt-4 text-[0.9375rem] leading-relaxed text-ink-soft">{item.description}</p>{item.externalUrl && <a className="mt-4 inline-block text-sm font-semibold text-accent hover:underline" href={item.externalUrl} target="_blank" rel="noopener noreferrer">View opportunity ↗ <span className="sr-only">(opens in a new tab)</span></a>}</article>)}</div> : <EmptyState title="No open opportunities" description="New calls and programmes will appear here when they are added." />}
      </section>
