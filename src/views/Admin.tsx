...
type AdminView = 'dashboard' | 'publications' | 'use-cases' | 'media' | 'analytics' | 'users';
...
const navGroups: { label: string; items: { label: string; view: AdminView; icon: LucideIcon }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { label: 'Datasets', view: 'publications', icon: Database },
      { label: 'Events', view: 'publications', icon: CalendarDays },
    ],
  },
  {
    label: 'Operations',
            ...
            <div key={group.label} className="mb-6">
              ...
              <ul className="space-y-1">
                {group.items.map((item) => {
                  ...
                  const isActive = activeView === item.view && !(item.label === 'Datasets' || item.label === 'Events');
                  return (
                    <li key={item.label}>
...
function Dashboard({ onNavigate, onAddPublication }: { onNavigate: (view: AdminView) => void; onAddPublication: () => void }) {
  return <div className="space-y-7">
    <PageHeading title="Dashboard" description="Overview of the Asia AI4D Observatory website and content activity." action={<div className="flex flex-wrap gap-2"><button type="button" onClick={onAddPublication} className="inline-flex min-h-[42px] items-center gap-2 rounded bg-accent px-4 text-sm font-medium text-white hover:bg-accent-dark"><Plus className="h-4 w-4" />Add publication</button><button type="button" onClick={() => onNavigate('use-cases')} className="inline-flex min-h-[42px] items-center gap-2 rounded border border-line-strong bg-surface px-4 text-sm font-medium text-ink-soft hover:bg-raised"><Plus className="h-4 w-4" />Add use case</button></div>} />
...
function PublicationsView({ initialEditor = false, onCloseEditor }: { initialEditor?: boolean; onCloseEditor?: () => void }) {
  const [editorOpen, setEditorOpen] = useState(initialEditor);
  ...
  if (editorOpen) return <PublicationEditor onBack={() => { setEditorOpen(false); onCloseEditor?.(); }} />;
  return <div className="space-y-6"><PageHeading title="Publications" description="Manage reports, briefs, working papers, and other research outputs." action={<button type="button" onClick={() => setEditorOpen(true)} className="inline-flex min-h-[42px] items-center gap-2 self-start rounded bg-accent px-4 text-sm font-medium text-white hover:bg-accent-dark"><Plus className="h-4 w-4" />Add publication</button>} /><div className="border border-line bg-surface"><FilterBar search={search} onSearch={setSearch} placeholder="Search publications"><SelectFilter label="Status" /><SelectFilter label="Type" /><SelectFilter label="Country" /><SelectFilter label="Topic" /><button type="button" className="inline-flex min-h-[40px] items-center gap-2 rounded border border-line bg-surface px-3 text-sm font-medium text-ink-soft hover:bg-raised"><ChevronDown className="h-4 w-4" />More filters</button></FilterBar><div className="flex items-center justify-between border-b border-line px-4 py-3 text-xs text-ink-muted"><span>{filtered.length} of {publications.length} publications</span><button type="button" className="font-medium text-accent hover:underline">Export</button></div>{filtered.length > 0 ? <div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="bg-raised/65 text-xs text-ink-muted"><tr><th className="w-10 px-5 py-3"><input type="checkbox" aria-label="Select all publications" /></th><th className="px-3 py-3 font-medium">Title</th><th className="px-3 py-3 font-medium">Type</th><th className="px-3 py-3 font-medium">Countries</th><th className="px-3 py-3 font-medium">Topic</th><th className="px-3 py-3 font-medium">Publication date</th><th className="px-3 py-3 font-medium">Status</th><th className="px-5 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody className="divide-y divide-line">{filtered.map((item) => <tr key={item.title} className="hover:bg-raised/40"><td className="px-5 py-4"><input type="checkbox" aria-label={`Select ${item.title}`} /></td><td className="max-w-[260px] px-3 py-4 font-medium text-ink">{item.title}</td><td className="px-3 py-4 text-ink-soft">{item.type}</td><td className="px-3 py-4 text-ink-soft">{item.countries}</td><td className="px-3 py-4 text-ink-soft">{item.topic}</td><td className="px-3 py-4 text-xs text-ink-muted">{item.date}</td><td className="px-3 py-4"><StatusBadge status={item.status} /></td><td className="px-5 py-4 text-right"><button type="button" className="rounded p-1.5 text-ink-muted hover:bg-raised hover:text-ink" aria-label={`More actions for ${item.title}`}><MoreHorizontal className="h-4 w-4" /></button></td></tr>)}</tbody></table></div> : <div className="px-6 py-16 text-center"><FileText className="mx-auto h-8 w-8 text-ink-muted" /><h2 className="mt-4 text-base font-semibold text-ink">No publications found</h2><p className="mt-2 text-sm text-ink-soft">Try a different search or clear the active filters.</p></div>}<div className="flex items-center justify-between border-t border-line px-5 py-3 text-xs text-ink-muted"><span>Showing 1–{filtered.length} of {publications.length}</span><div className="flex gap-1"><button type="button" disabled className="inline-flex h-8 w-8 items-center justify-center rounded border border-line text-ink-muted disabled:opacity-40" aria-label="Previous page"><ChevronLeft className="h-4 w-4" /></button><button type="button" className="inline-flex h-8 w-8 items-center justify-center rounded border border-line text-ink-soft hover:bg-raised" aria-label="Next page"><ChevronRight className="h-4 w-4" /></button></div></div></div></div>;
}

function PublicationEditor({ onBack }: { onBack: () => void }) {
  return <div className="space-y-6"><div className="flex flex-col gap-4 border-b border-line pb-5 sm:flex-row sm:items-center sm:justify-between"><div><button type="button" onClick={onBack} className="mb-3 inline-flex items-center gap-1 text-xs font-medium text-accent hover:underline"><ChevronLeft className="h-4 w-4" />Back to publications</button><h1 className="text-[1.75rem] font-semibold tracking-[-0.04em] text-ink">Add publication</h1><p className="mt-2 text-sm text-ink-soft">Create a structured research record for the public observatory.</p></div><div className="flex gap-2"><button type="button" onClick={onBack} className="min-h-[42px] rounded border border-line-strong bg-surface px-4 text-sm font-medium text-ink-soft hover:bg-raised">Save draft</button><button type="button" className="min-h-[42px] rounded bg-accent px-4 text-sm font-medium text-white hover:bg-accent-dark">Publish</button></div></div><div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_280px]"><div className="space-y-6"><section className="border border-line bg-surface p-5"><h2 className="text-base font-semibold text-ink">Basic information</h2><div className="mt-5 space-y-5"><label className="block"><span className="block text-sm font-medium text-ink">Publication title <span className="text-[#a04c43]">*</span></span><input className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink outline-none focus:border-accent" defaultValue="" placeholder="Enter a clear public title" /></label><div className="grid gap-5 sm:grid-cols-2"><label className="block"><span className="block text-sm font-medium text-ink">Publication type <span className="text-[#a04c43]">*</span></span><select className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink outline-none focus:border-accent" defaultValue=""><option value="" disabled>Select type</option><option>Report</option><option>Policy brief</option><option>Research brief</option><option>Mapping study</option></select></label><label className="block"><span className="block text-sm font-medium text-ink">Publication date</span><input type="date" className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink outline-none focus:border-accent" /></label></div><label className="block"><span className="block text-sm font-medium text-ink">Short summary <span className="text-[#a04c43]">*</span></span><textarea rows={3} className="mt-2 block w-full resize-y rounded border border-line-strong bg-canvas px-3 py-3 text-sm text-ink outline-none focus:border-accent" placeholder="Summarise the publication in two or three sentences." /></label></div></section><section className="border border-line bg-surface p-5"><h2 className="text-base font-semibold text-ink">Classification</h2><p className="mt-1 text-sm text-ink-soft">Use consistent relationships so the research remains discoverable.</p><div className="mt-5 grid gap-5 sm:grid-cols-2"><label className="block"><span className="block text-sm font-medium text-ink">Countries</span><input className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink outline-none focus:border-accent" placeholder="Search countries" /></label><label className="block"><span className="block text-sm font-medium text-ink">Topics</span><input className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink outline-none focus:border-accent" placeholder="Search topics" /></label><label className="block"><span className="block text-sm font-medium text-ink">Authors</span><input className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink outline-none focus:border-accent" placeholder="Search existing people" /></label><label className="block"><span className="block text-sm font-medium text-ink">Related content</span><input className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink outline-none focus:border-accent" placeholder="Search publications and use cases" /></label></div></section><section className="border border-line bg-surface p-5"><h2 className="text-base font-semibold text-ink">File and SEO</h2><div className="mt-5 grid gap-5 sm:grid-cols-2"><label className="block"><span className="block text-sm font-medium text-ink">Publication file</span><button type="button" className="mt-2 flex min-h-[92px] w-full flex-col items-center justify-center gap-2 rounded border border-dashed border-line-strong bg-canvas px-3 text-sm text-ink-muted hover:border-accent hover:text-accent"><FileText className="h-5 w-5" />Upload PDF</button></label><label className="block"><span className="block text-sm font-medium text-ink">SEO description</span><textarea rows={4} className="mt-2 block w-full resize-y rounded border border-line-strong bg-canvas px-3 py-3 text-sm text-ink outline-none focus:border-accent" placeholder="Optional description for search and sharing." /></label></div></section></div><aside className="h-fit border border-line bg-surface p-5 xl:sticky xl:top-[100px]"><h2 className="text-base font-semibold text-ink">Publishing</h2><div className="mt-5 space-y-5"><label className="block"><span className="block text-sm font-medium text-ink">Status</span><select className="mt-2 block min-h-[44px] w-full rounded border border-line-strong bg-canvas px-3 text-sm text-ink" defaultValue="Draft"><option>Draft</option><option>Ready for review</option><option>Scheduled</option><option>Published</option></select></label><div className="border-t border-line pt-5"><p className="text-xs font-semibold uppercase tracking-[0.08em] text-ink-muted">Content check</p><ul className="mt-3 space-y-3 text-sm"><li className="flex gap-2 text-[#28613e]"><ShieldCheck className="h-4 w-4 shrink-0" />Title added</li><li className="flex gap-2 text-ink-muted"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-line-strong" />Country selected</li><li className="flex gap-2 text-ink-muted"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-line-strong" />Topic selected</li><li className="flex gap-2 text-[#8b641c]"><span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-[#c3903c]" />SEO description missing</li></ul></div><p className="border-t border-line pt-5 text-xs leading-relaxed text-ink-muted">Required fields are marked with an asterisk. A draft can be saved without publishing.</p></div></aside></div></div>;
}
...
function UseCasesView() {
  ...
  return <div className="space-y-6"><PageHeading title="Responsible AI use cases" description="Document practical examples of responsible AI across Asia." action={<button type="button" className="inline-flex min-h-[42px] items-center gap-2 self-start rounded bg-accent px-4 text-sm font-medium text-white hover:bg-accent-dark"><Plus className="h-4 w-4" />Add use case</button>} /><div className="border border-line bg-surface"><FilterBar search={search} onSearch={setSearch} placeholder="Search use cases"><SelectFilter label="Country" /><SelectFilter label="Sector" /><SelectFilter label="Status" /><SelectFilter label="Topic" /></FilterBar><div className="overflow-x-auto"><table className="w-full min-w-[740px] text-left text-sm"><thead className="bg-raised/65 text-xs text-ink-muted"><tr><th className="px-5 py-3 font-medium">Title</th><th className="px-3 py-3 font-medium">Country</th><th className="px-3 py-3 font-medium">Sector</th><th className="px-3 py-3 font-medium">Organization</th><th className="px-3 py-3 font-medium">Status</th><th className="px-3 py-3 font-medium">Updated</th><th className="px-5 py-3" /></tr></thead><tbody className="divide-y divide-line">{filtered.map((item) => <tr key={item.title} className="hover:bg-raised/40"><td className="max-w-[260px] px-5 py-4 font-medium text-ink">{item.title}</td><td className="px-3 py-4 text-ink-soft">{item.country}</td><td className="px-3 py-4 text-ink-soft">{item.sector}</td><td className="px-3 py-4 text-ink-soft">{item.organization}</td><td className="px-3 py-4"><StatusBadge status={item.status} /></td><td className="px-3 py-4 text-xs text-ink-muted">{item.updated}</td><td className="px-5 py-4 text-right"><MoreHorizontal className="ml-auto h-4 w-4 text-ink-muted" /></td></tr>)}</tbody></table></div></div></div>;
}
...
export function Admin() {
  ...
  const title = activeView === 'use-cases' ? 'Responsible AI use cases' : activeView === 'media' ? 'Media library' : activeView === 'analytics' ? 'Analytics overview' : activeView === 'users' ? 'Users & roles' : activeView === 'publications' ? 'Publications' : 'Dashboard';

  return <div className="flex min-h-[100dvh] bg-canvas text-ink"><Sidebar activeView={activeView} onNavigate={navigate} mobileOpen={mobileNavOpen} onClose={() => setMobileNavOpen(false)} /><div className="flex min-w-0 flex-1 flex-col"><Topbar title={title} onOpenMenu={() => setMobileNavOpen(true)} search={search} onSearch={setSearch} /><main id="admin-main" className="flex-1 px-5 py-7 sm:px-8 lg:px-10 lg:py-9"><div className="mx-auto max-w-[1440px]">{activeView === 'dashboard' && <Dashboard onNavigate={navigate} onAddPublication={() => { setActiveView('publications'); setEditorOpen(true); }} />}{activeView === 'publications' && <PublicationsView initialEditor={editorOpen} onCloseEditor={() => setEditorOpen(false)} />}{activeView === 'use-cases' && <UseCasesView />}{activeView === 'media' && <MediaView />}{activeView === 'analytics' && <AnalyticsView />}{activeView === 'users' && <UsersView />}</div></main></div></div>;
}
